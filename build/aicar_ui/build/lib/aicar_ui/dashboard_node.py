...
class DashboardNode(Node):
    def __init__(self):
        ...
        # Subscriptions
        self.create_subscription(Image, '/camera/image_raw', self._cam_cb, 5)
        self.create_subscription(Image, '/image_bev_binary', self._bev_cb, 5)
        self.create_subscription(Image, '/image_bev_color', self._bev_color_cb, 5)
    ...
    def _bev_color_cb(self, msg):
        try:
            img = self.bridge.imgmsg_to_cv2(msg, 'bgr8')
            with self._frame_lock:
                self._frame_bev_color = img
        except Exception:
            pass
...
class DashboardApp:
    ...
    def _update_images(self):
        ...
        if mode == 'all':
            ...
            frame_map = {'camera': cam, 'bev': bev, 'sign': sign, 'red': red, 'bev_color': self.node._frame_bev_color}
